// ========================================================================
// AppPipelinePage — a single, app-scoped surface showing the CI/CD pipeline
// for EVERY configuration across ALL of an app's configuration types.
// Routed at /apps/:appId/pipeline, it complements the per-type authoring
// page (AppConfigTypePage, at /apps/:appId/config/:configTypeId) by giving a
// cross-type view: one list, one set of filters, one place to validate,
// review, approve, and deploy — regardless of which configuration type a row
// belongs to.
//
// ZERO app-specific code: every literal is derived from the route param
// (appId) and the manifest (app.configurationTypes). It reuses the shared
// configurationCanvasApi, ApprovalSubmissionDialog, ReviewsDrawer, and the
// pipeline action helpers verbatim from AppConfigTypePage — this page is a
// cross-type lens over the exact same data and actions, not a new pipeline.
// ========================================================================
...
import { PipelineStatusBadge, PipelineTimeline } from '@/components/shared/Pipeline'
...
// ---------------------------------------------------------------------------
// Status presentation (human labels for filters/toasts — the badge itself is
// the shared PipelineStatusBadge, which has its own internal presentation).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Small presentational helpers (local copies of AppConfigTypePage's, kept
// page-local since neither is exported elsewhere)
// ---------------------------------------------------------------------------
...
const AppPipelinePage: React.FC = () => {
  ...
  // Tags / environments (for the Environment filter + the approval dialog).
  const [availableTags, setAvailableTags] = useState<PlatformTag[]>([])

  // Connections + each configuration type's declared target componentTypes,
  // keyed by entityType so the deploy prerequisite check works per-row across
  // every configuration type on the app, not just one.
  const [componentTypesByEntity, setComponentTypesByEntity] = useState<Record<string, string[]>>({})
  ...
  // Which rows have their pipeline stage timeline expanded.
  ...
  // Every configuration for this app, across ALL configuration types — the
  // whole reason this page exists. entityType is deliberately omitted.
  const fetchConfigurations = useCallback(async () => {
    if (!ready) return
    setIsLoading(true)
    setError(null)
    try {
      const data = await configurationCanvasApi.getAll({ toolType: appId })
  ...
  // Tags + components + EVERY configuration type's target componentTypes
  // (from the app detail), keyed by entityType.
  useEffect(() => {
    if (!ready) return
    let active = true
    ...
    appService
      ...
      .then((detail) => {
        ...
        for (const ct of detail.configurationTypes) map[ct.id] = ct.componentTypes ?? []
        setComponentTypesByEntity(map)
      })
      .catch(() => {
        if (active) setComponentTypesByEntity({})
      })
    return () => {
      active = false
    }
  ...
  // Registered connections whose type matches a given configuration type's
  // declared target componentTypes — mirrors AppConfigTypePage's single-type
  // check, generalized per-row across every configuration type on the app.
  const matchingComponentsFor = useCallback(
    (entityType: string): PlatformComponent[] => {
      const types = componentTypesByEntity[entityType] ?? []
      if (types.length === 0) return []
      return components.filter((c) => types.some((ct) => String(c.type ?? '').includes(ct)))
    },
    [componentTypesByEntity, components],
  )

  // Human-readable configuration type name for an entityType, from the manifest.
  const configTypeName = useCallback(
    (entityType: string) => app?.configurationTypes.find((ct) => ct.id === entityType)?.name ?? entityType,
    [app],
  )
  ...
  const [sortField, setSortField] = useState<'name' | 'type' | 'status' | 'updatedAt'>('updatedAt')
  ...
  const typeFilterOptions = useMemo(
    () =>
      Array.from(new Set(configurations.map((c) => c.entityType))).map((entityType) => ({
        value: entityType,
        label: configTypeName(entityType),
      })),
    [configurations, configTypeName],
  )
  ...
  const filteredConfigs = useMemo(() => {
    ...
    if (typeFilter) rows = rows.filter((c) => c.entityType === typeFilter)
    ...
    return [...rows].sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'type':
          av = configTypeName(a.entityType).toLowerCase(); bv = configTypeName(b.entityType).toLowerCase(); break
        case 'status':
  ...
  }, [configurations, search, statusFilter, typeFilter, envFilter, sortField, sortDir, configTypeName])
  ...
  useEffect(() => {
    setPage(1)
  }, [search, statusFilter, typeFilter, envFilter, sortField, sortDir])

  // --- Per-config actions (reused verbatim from AppConfigTypePage's handlers) --

  const handleEdit = useCallback(
    (config: ConfigurationCanvasListItem) => {
      // The config type page owns the canvas editor — this page is a
      // cross-type lens, not a second editor.
      navigate(`/apps/${appId}/config/${config.entityType}`)
    },
    [navigate, appId],
  )
  ...
  const deployBlockedReason = useCallback(
    (config: ConfigurationCanvasListItem): string | null => {
      ...
      if (matching.length === 0) {
        const types = componentTypesByEntity[config.entityType] ?? []
        const target = types.length > 0 ? types.join('/') : 'target'
        return `Register a ${target} connection to deploy`
      }
      return null
    },
    [matchingComponentsFor, componentTypesByEntity],
  )
  ...
  return (
    <AppShell app={app} navItems={navItems} activePath="/pipeline">
      <div className="space-y-6 p-4 sm:p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              The CI/CD pipeline for every configuration across all of {app.name}&apos;s configuration types.
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
        ...
        <div className="rounded-lg border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
          {isLoading ? (
            <CenteredSpinner label="Loading pipeline…" />
            ...
            <div className="flex flex-col items-center justify-center px-4 py-16 text-center">
              ...
              <p className="mb-6 max-w-md text-sm text-gray-500 dark:text-gray-400">
                {app.name} has no configurations yet. Create one from a configuration type to see its
                pipeline here.
              </p>
              {app.configurationTypes.length > 0 && (
                <div className="flex flex-wrap items-center justify-center gap-2">
                  {app.configurationTypes.map((ct) => (
                    <Link
                      key={ct.id}
                      to={`/apps/${app.appId}/config/${ct.id}`}
            ...
            <div className="px-4 py-12 text-center text-sm text-gray-500 dark:text-gray-400">
              No configurations match your search or filters.
            </div>
          ) : (
                        ...
                        <tr className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                          <td className="px-2 py-4">
                            <button
                              ...
                              aria-label={
                                expanded
                                  ? `Hide pipeline stages for ${config.name}`
                                  : `Show pipeline stages for ${config.name}`
                              }
                          ...
                          <td className="px-4 py-4">
                            <Badge variant="secondary" size="sm">
                              {configTypeName(config.entityType)}
                            </Badge>
                          </td>
                          <td className="px-4 py-4">
                            <PipelineStatusBadge status={config.status} size="sm" />
                        ...
                        {expanded && (
                          <tr className="bg-gray-50 dark:bg-gray-900/30">
                            <td colSpan={8} className="px-6 py-5">
                              ...
                              <PipelineTimeline currentStatus={config.status} />
                            </td>
                          </tr>
                        )}
                      </React.Fragment>
...
export default AppPipelinePage
