...
interface SeedControl {
  ...
  description: string;
  requirement: string;
  category: string; // signal key: encryption | mfa | network | device | drift | approvals | auditLogging | accessControl | manual
}
...
async function seedComplianceCatalog(): Promise<void> {
  try {
